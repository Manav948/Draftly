...
const EditWorkspaceImage = ({ workspace: { id, color, name, image } }: Props) => {
    ...
    const imageOption = useMemo(() => {
        if (imagePreview) return { canDelete: false, canSave: true };
        if (image) return { canDelete: true, canSave: false };
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <div>
                ...
                <DialogTrigger asChild>
                    <Button
                        className={cn(
                            "w-16 h-16 font-semibold text-white rounded-md transition-all duration-200 cursor-pointer",
