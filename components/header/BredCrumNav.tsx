...
const BredCrumNav = ({ addManualRoutes, workspaceHref }: Props) => {
  ...
  if (pathNames.length > 0) {
    return (
            ...
            <nav className="flex items-center gap-2 text-sm text-black dark:text-white">
                    ...
                    {!isLast ? (
                      <>
                        {route.href ? (
                          ...
                          <span className="font-medium dark:text-red-500">
                            {route.useTranslate ? t(route.name) : route.name}
                          </span>
                        )}
              ...
              <span
                className={cn(
                  ...
                  "bg-gray-200/70 text-gray-900 dark:bg-white/80 dark:text-red-500 shadow-sm cursor-pointer"
                )}
              >
                {route.useTranslate ? t(route.name) : route.name}
              </span>
      ...
      <nav className="flex items-center gap-2 text-sm text-black dark:text-red-500">
            ...
            <div key={i} className="flex items-center gap-2">
              {!isLast ? (
                ...
                <span
                  className={cn(
                    "font-semibold relative px-2 py-1 rounded-md",
                    "bg-gray-200/70 text-gray-900 dark:bg-[#0c0c0c] dark:text-red-500 shadow-sm cursor-pointer"
                  )}
                >
                  {availableRoutesWithTranselation.includes(link) ? t(link.toUpperCase()) : link}
                </span>
