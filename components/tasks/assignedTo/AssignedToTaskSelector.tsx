...
import { UserAssigningToTaskInfo } from '@/types/extended'
...
const AssignedToTaskSelector = ({ className, plusIconSize = 16, dropDownsideOffSet, workspaceId, taskId }: Props) => {

    const router = useRouter()

    const { data: assignedToUserInfo, isLoading: isLoadingInfo } = useQuery({
        queryFn: async () => {
            const res = await fetch(`/api/assigned_to/tasks/get?workspaceId=${workspaceId}&taskId=${taskId}`)
            if (!res.ok) return {} as UserAssigningToTaskInfo;

            const data = await res.json()
            return data as UserAssigningToTaskInfo
        },
