...
const CommandUser = ({ user, mindMapId, workspaceId }: Props) => {
    const router = useRouter()
    const [isActiveUser, setIsActiveUser] = useState(
        user.user.assignedToMindMaps.length === 1 ? true : false
    )
    const queryClient = useQueryClient()

    const { mutate: hadndleTaskAssignment } = useMutation({
        mutationFn: async () => {
            await axios.post(`/api/assigned_to/mind_map/assign`, {
                mindMapId,
                workspaceId,
        ...
        onSuccess: () => {
            toast.success("MindMap Assigned SuccessFully")
            router.refresh()
        }, onSettled: () => {
            // @ts-ignore
