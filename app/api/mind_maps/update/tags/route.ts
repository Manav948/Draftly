...
import { mindMapSchema, updateMindMapSchema } from "@/schema/mindMapSchema";
...
export async function POST(request: Request) {
    ...
    const result = updateMindMapSchema.safeParse(body)

    if (!result.success) {
        return new NextResponse("Something went wrong", { status: 401 })
    }
    const { workspaceId, mindMapId, tagId } = result.data;
    ...
    try {
        ...
        const updatedMindMap = await db.mindMap.update({
            ...
            data: {
                ...
                tags: {
                    connect: tagId.map((tagId) => ({ id: tagId })),
                    disconnect: mindmap.tags.filter((existingTag) => !tagId.includes(existingTag.id))
                }
